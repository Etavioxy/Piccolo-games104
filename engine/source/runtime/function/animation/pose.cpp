...
void AnimationPose::blend(const AnimationPose& pose)
{
    for (int i = 0; i < m_bone_poses.size(); i++)
    {
        ...
        float sum_weight = m_weight.m_blend_weight[i] + pose.m_weight.m_blend_weight[i];//总权重为两个关键帧的权重之和
        if (sum_weight != 0)
        {
            float cur_weight = pose.m_weight.m_blend_weight[i] / sum_weight; //当前权重为要混合的那个pose的权重与总权重之比
